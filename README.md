...
By performing discrete rebalancing, we are essentially approximating the theoretical replication portfolio, whose holdings change continuously (an object that is plausible on the mathematical world but we cannot cope with in reality) with one that is rebalanced finitely often (just like we approximating an arbitrary function using a combination of step functions). The reason for the convergence of these discretely rebalanced portfolios is similar to why the euler method converges when solving a classical DE; the error can be bounded by the second derivative of the price with respect to the stock (gamma). A way to prove this is by showing that the accumulation of the local errors (proportional to the difference between implied and realised volatilities) converge almost surely to 0 as the rebalance frequency becomes large. The total error is therefore path dependent with zero expected value for any rebalancing frequency and a variance that goes to 0 as the frequency goes to infinity. Section I of [convergence_errors](https://pages.stern.nyu.edu/~lpederse/courses/LAP/papers/Derivatives/Leland85.pdf) does a good job at explaining the steps to prove the convergence of these errors
...
![hedge_error](https://github.com/user-attachments/assets/62adaeca-935d-4fb4-9fcb-7184856a8916)y 
